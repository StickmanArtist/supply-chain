...
export default function InfoPanel({ curItem, curCountry, activeTab, setActiveTab }) {
  return (
    <div style={{
      ...
      {/* 탭 */}
      <div style={{ display:'flex', borderBottom:'0.5px solid #ddd', overflowX:'auto' }}>
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            style={{
              flex:1, padding:'7px 2px', textAlign:'center', fontSize:'11px',
    ...
    if (curCountry) {
      return (
        <div style={{ padding:'20px', textAlign:'center', color:'#888', fontSize:'12px', lineHeight:'1.8' }}>
          국가 선택을 해제하면<br/>시세 차트를 볼 수 있습니다
        </div>
      );
    }
    ...
    <div>
      <Section>
        ...
        {[...curItem.regions].sort((a,b) => b.prod-a.prod).slice(0,4).map(r => (
          <BarRow key={r.name} label={r.name} value={r.prod} max={maxP} color="#E8593C"/>
        ))}
      </Section>
      <Section>
        <SectionLabel color="#3B8BD4" text="소비 상위국"/>
        {[...curItem.regions].sort((a,b) => b.cons-a.cons).slice(0,4).map(r => (
          <BarRow key={r.name} label={r.name} value={r.cons} max={maxC} color="#3B8BD4"/>
        ))}
      </Section>
      <Section>
        <SectionLabel color="#F2A623" text="핵심 기업"/>
...
function NewsTab({ curItem, curCountry }) {
  ...
  useEffect(() => {
    ...
    const url = `${API}/news/${curItem.id}${curCountry ? `?country=${encodeURIComponent(curCountry)}` : ''}`;
  ...
  if (loading) return <div style={{ padding:'20px', textAlign:'center', color:'#888', fontSize:'12px' }}>뉴스 검색 중...</div>;
  if (!news?.length) return <div style={{ padding:'20px', textAlign:'center', color:'#888', fontSize:'12px' }}>뉴스를 찾을 수 없습니다</div>;
    ...
    <div>
      {news.map((n, i) => (
        ...
        >
          ...
          <div style={{ fontSize:'12px', lineHeight:'1.45', marginBottom:'3px', fontWeight:'500' }}>{n.title}</div>
          <div style={{ fontSize:'11px', color:'#888', lineHeight:'1.5' }}>{n.summary}</div>
          {n.url && <div style={{ fontSize:'10px', color:'#3B8BD4', marginTop:'4px' }}>기사 읽기 →</div>}
        </div>
      ))}
    </div>
  );
...
function AITab({ curItem, curCountry }) {
  ...
  if (loading) return <div style={{ padding:'20px', textAlign:'center', color:'#888', fontSize:'12px' }}>분석 중...</div>;
  if (!result) return <div style={{ padding:'20px', textAlign:'center', color:'#888', fontSize:'12px' }}>분석 불가</div>;
...
function SmallBtn({ onClick, children }) {
  return (
    <button onClick={onClick} style={{ flex:1, padding:'7px', borderRadius:'7px', border:'0.5px solid #ddd', background:'transparent', fontSize:'11px', cursor:'pointer', fontFamily:'inherit' }}>
      {children}
    </button>
  );
}